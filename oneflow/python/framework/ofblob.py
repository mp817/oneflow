...
class OfBlob(object):
    ...
    def shape_list(self):
        ...
        while not oneflow_internal.OfBlob_CurTensorIteratorEqEnd(self.of_blob_ptr_):
            ...
            oneflow_internal.OfBlob_CurTensorCopyShapeTo(self.of_blob_ptr_, shape_tensor)
    ...
    def _CopyToNdarrayListAndIsNewSliceStartMask(self):
        ...
        tensor_list = []
        oneflow_internal.OfBlob_ResetTensorIterator(self.of_blob_ptr_)
        while oneflow_internal.OfBlob_CurTensorIteratorEqEnd(self.of_blob_ptr_) == False:
            shape_tensor = np.zeros(self.num_axes, dtype=np.int64)
            oneflow_internal.OfBlob_CurTensorCopyShapeTo(self.of_blob_ptr_, shape_tensor)
    ...
    def _CopyFromNdarrayListAndIsNewSliceStartMask(
        ...
        assert num_slices == oneflow_internal.OfBlob_NumOfTensorListSlices(self.of_blob_ptr_)
